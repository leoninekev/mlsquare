...
class PytorchLogisticRegression(GeneralizedLinearModel):
    def __init__(self):
        ...
        self.version = 'V2'
        self.modeling_backend = 'pytorch'
        model_params = {'layer_1': {'units': 1,
                                     'l1': 0,
                                     'l2': 0,
                                     'activation': tune.function(F.sigmoid)},
    ...
    def create_model(self, **kwargs):
        ...
        if len(self.y.shape) == 1 or self.y.shape[1] == 1:
            ...
            units = self.y.shape[1]
        model_params['layer_1'].update({'input_dim': self.X.shape[1], 'units': units})#self.X.shape[1]

        class network(nn.Module):
            def __init__(self):
                super(network, self).__init__()
                self.fc_layer = nn.Linear(model_params['layer_1']['input_dim'], model_params['layer_1']['units'])#100)

            def forward(self, X):
                X = model_params['layer_1']['activation'](self.fc_layer(X))#F.relu(self.fc1(X))
                return X
        self.model= network()
        self.optimizer = model_params['optimizer'](self.model.parameters())#optim.Adam(self.model.parameters())
        self.criterion= model_params['loss']#nn.BCELoss()#CrossEntropyLoss()
